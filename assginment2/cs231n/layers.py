...
def affine_forward(x, w, b):
    ...
    N = x.shape[0]    
    out = np.dot(x.reshape(N,-1), w) + b    # MxN        
...
def affine_backward(dout, cache):
    ...
    N = x.shape[0]
    dx = np.dot(dout, w.T).reshape(x.shape)   # (NxM) x (MxD) -> reshape -> (N, d_1, ..., d_k)
    dw = np.dot(x.reshape(N,-1).T, dout)    # (DxN) x (NxM)  -> (D,M)    
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    Input:
    - x: Data of shape (N, D)
    - gamma: Scale parameter of shape (D,)    표준편차
    - beta: Shift paremeter of shape (D,)    평균
    ...
    if mode == 'train':
        ...
        sample_mean = np.sum(x, axis=0, keepdims=True) / N    # (1, D)
        sample_var = np.sum((x - sample_mean)**2, axis=0, keepdims=True) / N    # (1, D)        
        ...
        cache = (x, x_hat, sample_mean, sample_var, gamma, beta, bn_param)    # bn_params는 update이전 게 필요한 건가?
...
def batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    x, x_hat, sample_mean, sample_var, gamma, beta, bn_param = cache   
    eps = bn_param.get('eps', 1e-5)
    ...
    gamma             (1,D)
    beta                 (1,D)
    bn_param          dictionary containing mode, eps, mementum, running_mean, running_var
    """
    
    dx_hat = dout * gamma    # (N,D) x (1,D) = (N, D)
    dvar = np.sum(dx_hat * (x - sample_mean), axis=0) * (-0.5 * np.power((sample_var+eps), -1.5))    # (N,D) x (1xD) / (1,D) = (N,D)        
    ######여기 틀린 거였음!!! dx_hat * (x-sample_mean)해줘야함. (x-sample_mean) * np.sum(dx_hat, axis=0)이 아니라!!
    din1 = dx_hat / np.sqrt(sample_var + eps)    # (N,D) x (1,D) = (N,D)      # 루트 안 씌우나??
    din2 = np.ones((N,D)) / N  * dvar * (2*(x-sample_mean))
    dx1 = din1 + din2
    dmu = np.sum(din1+din2, axis=0) * -1 
